...
registerDoMC(100)
arg<-commandArgs(T)
parameters<-expand.grid(c("additive","dominant","recessive"),c("1G","2G"),c(0,0.8))
...
for(PB in FreqB){
  ...
  pvalues<-foreach(i=c(1:Niter),.combine = "rbind")%dopar%{
    ...
    if(e=="1G" & m=="additive"){lg<- b0+b1*P$G2}
    if(e=="1G" & m=="recessive"){lg<- b0+b1*((P$G2>1)+0)}
    if(e=="1G" & m=="dominant"){lg<- b0+b1*((P$G2>0)+0)}
    if(e=="2G" & m=="additive"){lg<- b0+b1*P$G1+b1*P$G2}
    if(e=="2G" & m=="recessive"){lg<- b0+b1*((P$G1>1)+0)+b1*((P$G2>1)+0)}
    if(e=="2G" & m=="dominant"){lg<- b0+b1*((P$G1>0)+0)+b1*((P$G2>0)+0)}
    ...
    fit.joint<-glm(Y~G1+G1.D+G2+G2.D,data=P,family = binomial())
    p_r_joint<-lrtest(fit,fit.joint)$`Pr(>Chisq)`[2]
    #Saturate
    ...
    fiti1<-glm(Y~G1+G2+G1*G2+G1*G2.D+G1.D*G2+G1.D*G2.D,data=P,family = binomial())
  ...
  power_r_joint<-append(power_r_joint,sum(pvalues[,4]<0.05)/dim(pvalues)[1])
...
P<-data.frame(FreqB,
              power_r_cis,power_r_trans,power_s,power_r_joint,power_wu,power_h,power_i,
              Dp=rep(Dp,length(FreqB)),model=rep(m,length(FreqB)),effect=rep(e,length(FreqB)))

write.table(P,paste0("T1E_","PA_",PA,"_",e,"_",m,"_",Dp,".txt"),quote=F,col.names = T,row.names = F)
